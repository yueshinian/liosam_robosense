...
std::string pcl_input="/rslidar_points";
std::string pcl_output="/points_raw";
...
int main(int argc, char **argv) {
    ...
    nhPrivate.getParam("pcl_input",pcl_input);
    nhPrivate.getParam("pcl_output",pcl_output);
    /*
    if (argc == 1) {
    ...
    */
    subRobosensePC = nh.subscribe(pcl_input, 1, rsHandler_XYZIRT);
    pubRobosensePC = nh.advertise<sensor_msgs::PointCloud2>(pcl_output, 1);

    ROS_INFO("Listening to /rslidar_points ......");
    ROS_INFO("\033[1;32m----> rs_to_velodyne Started.\033[0m");
    ros::spin();
    return 0;
}
